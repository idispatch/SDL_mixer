...
Mix_Music *Mix_LoadMUS(const char *file) {
    ...
    if (ext) {
        ...
        if (MIX_string_equals(ext, "WAV")) {
            type = MUS_WAV;
        } else if (MIX_string_equals(ext, "MID") || MIX_string_equals(ext, "MIDI")
            ...
            type = MUS_FLAC;
        } else if (MIX_string_equals(ext, "MPG") || MIX_string_equals(ext, "MPEG")
                || MIX_string_equals(ext, "MP3") || MIX_string_equals(ext, "MAD")) {
            type = MUS_MP3;
        }
    }
    if (type == MUS_NONE) {
     ...
     * generic one, so we clear the current one. */Mix_SetError("");
...
static void music_internal_halt(void) {
    ...
    skip: music_playing->fading = MIX_NO_FADING;
    music_playing = NULL;
}
...
# endif
    ...
    /* rcg06042009 report available decoders at runtime. */SDL_free(music_decoders);
    music_decoders = NULL;
    num_decoders = 0;
